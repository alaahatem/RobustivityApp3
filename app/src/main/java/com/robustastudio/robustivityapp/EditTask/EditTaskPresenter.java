...
public class EditTaskPresenter implements EditTaskPresenterInt {
    ...
    @Override
    public void taskInfo(AppDatabase db, String id) {
    ...
    public void editTask(AppDatabase db, Tasks task) {
        ...
        switch (task.getStartDate().getMonth()) {
            ...
            case 9:
                ;
            case 11:
                if (task.getStartDate().getDay() > 30) {
        ...
        switch (task.getDue_date().getMonth()) {
            ...
            case 10:
                ;
            case 12:
                if (task.getDue_date().getDay() > 31) {
                    Toast.makeText(view, "Day cant exceed 31 at Start Date", Toast.LENGTH_LONG).show();
                    flag = true;
                }
                break;
            case 2:
                if (task.getDue_date().getDay() > 29) {
        ...
        if (!flag) {
            Tasks tempo=db.taskDao().getUser(task.getId());
            task.setAssignee(tempo.getAssignee());
